...
public class GeoShape implements Serializable {
	...
	public static GeoShape multiLineString(List<GeoPoint>... points) {
		return GeoShape.of(Type.MultiLineString,
				Arrays.stream(points).map(GeoShape::toPointList).collect(Collectors.toList()));
	}
	...
	public static GeoShape multiPolygon(List<List<GeoPoint>>... points) {
		return GeoShape.of(Type.MultiPolygon,
				Arrays.stream(points).map(list -> list.stream().map(GeoShape::toPointList).collect(Collectors.toList()))
						.collect(Collectors.toList()));
	}

	public static GeoShape collection(GeoShape... shapes) {
		return GeoShape.of(Type.GeometryCollection,
				Arrays.stream(shapes).map(GeoShape::toMap).collect(Collectors.toList()));
	}

	private static List<Object> toPointList(Iterable<GeoPoint> points) {

		return StreamSupport.stream(points.spliterator(), false).map(p -> Arrays.asList(p.getLon(), p.getLat()))
				.collect(Collectors.toList());
	}
