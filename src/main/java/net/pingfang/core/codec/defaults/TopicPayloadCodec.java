...
public class TopicPayloadCodec implements Codec<TopicPayload> {
	...
	public static ByteBuf doEncode(TopicPayload body) {
		...
		try {
			...
			return ByteBufAllocator.DEFAULT.buffer(topicLen.length + topic.length + bodyBuf.writerIndex())
					.writeBytes(topicLen).writeBytes(topic).writeBytes(bodyBuf, 0, bodyBuf.writerIndex());
		} catch (Throwable e) {
	...
	public Payload encode(TopicPayload body) {
		...
		try {
			ByteBuf bodyBuf = body.getBody();
			return Payload.of(ByteBufAllocator.DEFAULT.buffer(topicLen.length + topic.length + bodyBuf.writerIndex())
					.writeBytes(topicLen).writeBytes(topic).writeBytes(bodyBuf, 0, bodyBuf.writerIndex()));
		} catch (Throwable e) {
			log.error("encode topic [{}] payload error", body.getTopic());
			throw e;
