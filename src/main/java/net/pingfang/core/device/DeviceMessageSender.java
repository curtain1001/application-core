...
public interface DeviceMessageSender {
	 ...
	 * 发送一个支持回复的消息.
	 * <p>
	 * ⚠️: 请勿自己实现消息对象,而应该使用框架定义的3种消息. ⚠️:
	 * 如果是异步消息,将直接返回<code>{"success":true,"code":"REQUEST_HANDLING"}</code>
	 *
	 ...
	 * @seeDeviceMessageSenderInterceptor
	 */
	 ...
	 */
	<R extends DeviceMessage> Flux<R> send(Publisher<? extends DeviceMessage> message,
	...
	default Mono<Void> sendAndForget(DeviceMessage message) {
		return this.send(message.addHeader(Headers.async, true).addHeader(Headers.sendAndForget, true)).then();
	}
