...
public class EnumType extends AbstractType<EnumType> implements DataType {
	...
	public ValidateResult validate(Object value) {
		...
		return elements.stream().filter(ele -> match(value, ele)).findFirst().map(e -> ValidateResult.success(e.value))
	...
	public String format(Object value) {
		...
		return elements.stream().filter(ele -> ele.value.equals(String.valueOf(value))).findFirst()
				.map(Element::getText).orElse(stringVal);
	}
